...
# --- AHK INTEGRATION ---
ahk_process = None
ahk_script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mouse_control.ahk")

def start_ahk():
    """Start AutoHotkey script"""
    global ahk_process
    if not os.path.exists(ahk_script_path):
        print("⚠️  Warning: mouse_control.ahk not found! Mouse control may not work.")
        print(f"📁 Looking for: {ahk_script_path}")
        return False
    
    try:
        # Check for custom AHK path from environment variable
        custom_ahk = os.environ.get('AHK_PATH')
        if custom_ahk and os.path.exists(custom_ahk):
            print(f"✅ Using custom AHK path from AHK_PATH: {custom_ahk}")
            ahk_exe = custom_ahk
        else:
            # Try to find AutoHotkey executable - check many locations
            ahk_paths = [
                ...
                os.path.join(os.getenv('LOCALAPPDATA', ''), 'Programs', 'AutoHotkey', 'AutoHotkey.exe'),
                os.path.join(os.getenv('PROGRAMFILES', ''), 'AutoHotkey', 'AutoHotkey.exe'),
            ]
            ...
            if path_ahk:
                ahk_paths.insert(0, path_ahk)
            
            ahk_exe = None
            print("\n🔍 Searching for AutoHotkey...")
            ...
            if not ahk_exe:
                ...
                print("="*60)
                print("📥 Please install AutoHotkey v1.1 from:")
                print("   https://www.autohotkey.com/download/ahk-install.exe")
                print("\n💡 After installing, restart this script.")
                print("\n🔧 If already installed, you can specify the path:")
                print("   set AHK_PATH=C:\\path\\to\\AutoHotkey.exe")
                print("="*60 + "\n")
                return False
        
        ahk_process = subprocess.Popen([ahk_exe, ahk_script_path])
        time.sleep(0.5)  # Give AHK time to start
        print(f"✅ AutoHotkey started successfully (PID: {ahk_process.pid})")
        return True
    except Exception as e:
        print(f"❌ Failed to start AutoHotkey: {e}")
        import traceback
        traceback.print_exc()
        return False

def stop_ahk():
    """Stop AutoHotkey script"""
    global ahk_process
    if ahk_process:
        send_ahk_command("exit")
        time.sleep(0.2)
        ahk_process.terminate()
        print("🛑 AutoHotkey stopped")

def send_ahk_command(command, *args):
    """Send command to AutoHotkey via temporary file"""
    try:
        cmd_str = command + ("|" + "|".join(map(str, args)) if args else "")
        with open("ahk_command.tmp", "w") as f:
            f.write(cmd_str)
        time.sleep(0.01)  # Small delay for AHK to process
        return True
    except Exception as e:
        print(f"AHK Command Error: {e}")
        return False

def get_ahk_response(timeout=0.5):
    """Get response from AutoHotkey"""
    start = time.time()
    while time.time() - start < timeout:
        if os.path.exists("ahk_response.tmp"):
            try:
                with open("ahk_response.tmp", "r") as f:
                    response = f.read()
                os.remove("ahk_response.tmp")
                return response
            except:
                pass
        time.sleep(0.01)
...
def get_mouse_position():
    """Get current mouse position using win32api as fallback"""
    try:
        import win32api
        pos = win32api.GetCursorPos()
        return pos[0], pos[1]
    except:
        # Fallback to reading from screen center
        return SCREEN_W // 2, SCREEN_H // 2

# Start AHK on app startup
ahk_available = start_ahk()
...
<body>
    ...
    <div id="status-bar">
        ...
        <span id="ahk-display" class="ahk-status">AHK</span>
    ...
    <div id="settings-panel">
        ...
        <div class="setting-item">
            ...
            <select id="quality-select" style="width: 100%; padding: 8px; background: #111; color: white; border: 1px solid #444; border-radius: 5px;">
    ...
    <script>
        ...
        // Check AHK status
        fetch('/ahk_status').then(r => r.json()).then(data => {
            const ahkDisplay = document.getElementById('ahk-display');
            if (data.available) {
                ahkDisplay.textContent = '✓ AHK';
                ahkDisplay.classList.remove('disabled');
            } else {
                ahkDisplay.textContent = '✗ AHK';
                ahkDisplay.classList.add('disabled');
            }
        });
        ...
        fetch('/get_volume').then(r => r.json()).then(data => {
            if (data.volume !== null) {
                const vol = Math.round(data.volume);
        ...
        function toggleFullscreen() {
            ...
            // Load saved button positions when entering fullscreen
            if (isFullscreen) {
                loadButtonPositions();
            }
            
            // Request fullscreen API with cross-browser support
        ...
        document.addEventListener('click', function(e) {
            if (!isFullscreen || isEditMode) return; // Don't move mouse in edit mode!
            
            const streamFs = document.getElementById('stream-fs');
            const viewer = streamFs ? streamFs.closest('.fs-viewer') : null;
            
            // Check if click is on the video stream (not on buttons)
            if (streamFs && (e.target === streamFs || e.target === viewer)) {
        ...
        function makeDraggable(element) {
            ...
            function dragMouseDown(e) {
                e.preventDefault();
                e.stopPropagation(); // Prevent click-to-move
...
@app.route('/ahk_status')
def ahk_status():
    return jsonify({'available': ahk_available})
...
def get_volume():
    ...
    if ahk_available:
        send_ahk_command("get_volume")
        vol = get_ahk_response()
        if vol:
            try:
                return jsonify({'volume': float(vol)})
            except:
                pass
    ...
    def gen():
        ...
        with mss.mss() as sct:
            ...
            while True:
                ...
                pos_x, pos_y = get_mouse_position()
...
def zoom_in():
    ...
    zoom_center_x, zoom_center_y = get_mouse_position()
...
def set_volume(val):
    if ahk_available:
        send_ahk_command("set_volume", val)

def scroll_up():
    if ahk_available:
        send_ahk_command("scroll_up", 3)

def scroll_down():
    if ahk_available:
        send_ahk_command("scroll_down", 3)

def type_text(text):
    """Type text using AHK SendInput for better compatibility"""
    if ahk_available:
        # For now, use a simple character-by-character approach
        # In production, you might want to enhance the AHK script to handle this
        for char in text:
            if char == ' ':
                send_ahk_command("key", "Space")
            elif char == '\n':
                send_ahk_command("key", "Enter")
            else:
                # For simplicity, just send the character
                # AHK script would need to be enhanced for full keyboard support
                pass
        time.sleep(0.01 * len(text))

ACTIONS = {
    "click": lambda: (send_ahk_command("click") if ahk_available else None, mark_click()),
    "right_click": lambda: (send_ahk_command("right_click") if ahk_available else None, mark_click()),
    "drag_start": lambda: send_ahk_command("mouse_down") if ahk_available else None,
    "drag_end": lambda: send_ahk_command("mouse_up") if ahk_available else None,
    "type": type_text,
    "move_joy": lambda x, y: send_ahk_command("move_rel", float(x)*2, float(y)*2) if ahk_available else None,
    "move_abs": lambda x, y: send_ahk_command("move_abs", float(x), float(y)) if ahk_available else None,
    "zoom_in": zoom_in,
    "zoom_out": zoom_out,
...
if __name__ == '__main__':
    print("=" * 50)
    print("🖥️  Enhanced Remote Desktop Server (AHK Edition)")
    print("=" * 50)
    print(f"🌐 Access at: http://0.0.0.0:5000")
    print(f"🔒 Default password: 1234")
    print(f"📱 Screen size: {SCREEN_W}x{SCREEN_H}")
    print(f"🎮 Mouse control: {'AutoHotkey' if ahk_available else 'DISABLED - AHK not found'}")
    print(f"🔊 Audio control: {'AutoHotkey' if ahk_available else 'DISABLED'}")
    if not ahk_available:
        print("⚠️  Install AutoHotkey from https://www.autohotkey.com/")
    print("=" * 50)
